...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=textwrap.dedent('''\
    ...
    patches from AOSiP Gerrit instance.
...
def fetch_query(query):
    url = 'http://review.aosip.xyz/changes/?q=%s' % query
    if args.verbose:
...
for changeps in args.change_number:
    ...
    #   )]}'
    #   [ ... valid JSON ... ]
    url = 'http://review.aosip.xyz/changes/?q={change}&o={query_revision}&o=CURRENT_COMMIT&pp=0'.format(change=change, query_revision=query_revision)
    if args.verbose:
